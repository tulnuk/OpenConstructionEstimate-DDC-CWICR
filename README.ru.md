...
Папка `AI_INSTRUCTIONS/` содержит комплексную документацию для AI-ассистентов программирования для эффективной работы с этой базой данных строительных расценок.

### Что такое DDC CWICR?

**DDC CWICR** (Construction Work Items, Components & Resources) — это открытая база данных строительных расценок, содержащая:
- **55 719 позиций работ** — детальные строительные операции с полной разбивкой стоимости
- **27 672 ресурса** — материалы, трудозатраты и оборудование с региональными ценами
- **85 полей данных** — структурированная схема для точных расчётов
- **9 языков** — с региональными ценами (EUR, USD, CAD, RUB, CNY и др.)
- **Предрассчитанные эмбеддинги** — 3072-мерные векторы OpenAI для семантического поиска

### Ресурсный метод расчёта

База данных использует **ресурсный подход к калькуляции**, разделяющий:
- **Технологические нормы** (неизменные) — трудозатраты, расход материалов, время работы техники
- **Региональные цены** (изменяемые) — почасовые ставки, цены материалов, стоимость топлива

```
Фактическая стоимость = Технологическая норма × Региональная цена
```

Это обеспечивает точную оценку для разных регионов и периодов времени.
...
Включённые n8n workflows — это **примеры и шаблоны**, демонстрирующие логику расчёта смет. Их можно:
- ✅ Использовать как есть для быстрого развёртывания
- ✅ Частично адаптировать под конкретные бизнес-требования
- ✅ Изучать для понимания методологии расчёта стоимости
- ✅ Использовать как референс при создании интеграций на любой платформе

Workflows демонстрируют: запросы к базе, сопоставление позиций, логику ценообразования и генерацию отчётов. AI может анализировать их для понимания полного процесса калькуляции.

### Почему это важно

AI-ассистенты помогут вам:
- Запрашивать базу данных на естественном языке
- Находить работы через семантический поиск
- Рассчитывать стоимость с региональными ценами
- Генерировать отчёты и экспортировать данные
- Создавать интеграции с облачными сервисами
- Понимать методологию расчёта из примеров workflows

### Быстрый старт с AI

1. Откройте проект в IDE с поддержкой AI
2. Спросите: *"Покажи все работы по бетону с их стоимостью"*
3. AI использует инструкции для корректного запроса данных

**Книга**: Подробности методологии см. в [книге Data-Driven Construction](https://datadrivenconstruction.io/book)
